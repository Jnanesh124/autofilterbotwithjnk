        ...
        if settings['fsub'] != None:
            try:
                ...
                if btn:
                    ...
                    await message.reply_photo(photo=random.choice(PICS), caption=f"👋 Hello {message.from_user.mention},\n\nPlease join the channel then click on unmute me button. 😇", reply_markup=InlineKeyboardMarkup(btn))
                    return
...
async def pm_text(bot, message):
    ...
    from database.filtered_words_db import is_word_filtered
    if await is_word_filtered(content):
        return
...
async def next_page(bot, query):
   ...
   # if not search:
    ...
    if settings['button']:
        ...
        btn = []
    try:
        max_buttons = 10 if settings['max_btn'] else int(MAX_B_TN)
        if 0 < offset <= max_buttons:
            off_set = 0
        elif offset == 0:
            off_set = None
        else:
            off_set = offset - max_buttons

        if n_offset == 0:
            btn.append(
                [InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"), InlineKeyboardButton(f"{math.ceil(int(offset)/max_buttons)+1} / {math.ceil(total/max_buttons)}", callback_data="pages")]
            )
        elif off_set is None:
            btn.append([InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(f"{math.ceil(int(offset)/max_buttons)+1} / {math.ceil(total/max_buttons)}", callback_data="pages"), InlineKeyboardButton("𝐍𝐄𝐗𝐓 ➪", callback_data=f"next_{req}_{key}_{n_offset}")])
        else:
            btn.append(
                [
                    InlineKeyboardButton("⌫ 𝐁𝐀𝐂𝐊", callback_data=f"next_{req}_{key}_{off_set}"),
                    InlineKeyboardButton(f"{math.ceil(int(offset)/max_buttons)+1} / {math.ceil(total/max_buttons)}", callback_data="pages"),
...
async def filter_yearss_cb_handler(client: Client, query: CallbackQuery):
    ...
    if offset != "":
        try:
            max_buttons = 10 if settings['max_btn'] else int(MAX_B_TN)
            btn.append(
                [InlineKeyboardButton("𝐏𝐀𝐆𝐄", callback_data="pages"), InlineKeyboardButton(text=f"1/{math.ceil(int(total_results)/max_buttons)}",callback_data="pages"), InlineKeyboardButton(text="𝐍𝐄𝐗𝐓 ➪",callback_data=f"next_{req}_{key}_{offset}")]
            )
        except KeyError:
            await save_group_settings(query.message.chat.id, 'max_btn', True)
            btn.append(
